...
class TotalScore(Base):
    ...
    email = Column(String(32), ForeignKey("Users.email"), unique=True, nullable=False)
    ...
    user = relationship("User", backref="total_score")
...
class UserDailyScore(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    email = Column(String(32), ForeignKey("Users.user_id"), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint('email', 'score_date', 'start_time', 'subject', 'location', name='uix_user_score_detail')
    )
